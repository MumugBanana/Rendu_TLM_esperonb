...
void MBWrapper::exec_data_request(enum iss_t::DataAccessType mem_type,
	...
	case iss_t::READ_BYTE: {
		/* The ISS requested a data read (mem_addr into localbuf). */
		/* Get the offset of the byte we want to read
		 and then remove the offset from the addr we want to read */
		...
		/* Convert the data from machine to big endian
		 and select the byte we want to read from the word */
	...
	case iss_t::WRITE_WORD: {
		/* The ISS requested a datato be write
	...
	while (true) {
		if (m_iss.isBusy())
			...
			if (ins_asked) {
				...
				uint32_t localbuf;
				tlm::tlm_response_status status = socket.read(ins_addr,
						localbuf);
				if (status != tlm::TLM_OK_RESPONSE) {
					std::cerr << "Write error MBWrapper::exec_data_request "
							<< status;
				}
...
void MBWrapper::setIrq_true(void) {
	/* Check if irq_signal is positive */
	if (irq.read() == 1) {
		m_iss.setIrq(true);
	}
}
